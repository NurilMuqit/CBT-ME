                                            ...
                                            @foreach ($ujian->sortBy('id') as $u)
                                                <tr>
                                                    ...
                                                    <td>
                                                        @if ($u->ujian->jenis == 0)
                                                            @if ($isAccessible)
                                                                <a href="{{ url('siswa/ujian/' . $u->kode) }}"
                                                                    class="btn btn-primary btn-sm @if ($u->waktu_berakhir === null) btn-kerjakan @endif">
                                                                    <span data-feather="edit-3"></span> kerjakan</a>
                                                            @else
                                                                <a class="btn btn-danger" disabled>Ujian Belum Mulai</a>
                                                            @endif

                                                            @if ($u->waktu_berakhir)
                                                                @if ($u->selesai == null)
                                                                    <a href="{{ url('siswa/ujian/' . $u->kode) }}"
                                                                        class="btn btn-primary btn-sm @if ($u->waktu_berakhir === null) btn-kerjakan @endif"><span
                                                                            data-feather="edit-3"></span> lanjut
                                                                        kerjakan</a>
                                                                @else
                                                                    <a class="">selesai</a>
                                                                @endif
                                                        ...
                                                        @else
                                                            @if ($isAccessible)
                                                                <a href="{{ url('siswa/ujian_essay/' . $u->kode) }}"
                                                                    class="btn btn-primary btn-sm @if ($u->waktu_berakhir === null) btn-kerjakan @endif"><span
                                                                        data-feather="edit-3"></span> kerjakan</a>
                                                            @else
                                                                <a class="btn btn-danger" disabled>Ujian Belum Mulai</a>
                                                            @endif
